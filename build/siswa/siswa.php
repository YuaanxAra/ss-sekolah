...
<body>
  ...
  <aside class="fixed inset-y-0 flex-wrap items-center justify-between block w-full p-0 my-4 overflow-y-auto antialiased transition-transform duration-200 -translate-x-full bg-white border-0 shadow-xl dark:shadow-none dark:bg-slate-850 max-w-64 ease-nav-brand z-990 xl:ml-6 rounded-2xl xl:left-0 xl:translate-x-0" aria-expanded="false">
    ...
    <div class="items-center block w-auto max-h-screen overflow-auto h-sidenav grow basis-full">
      <ul class="flex flex-col pl-0 mb-0">
        ...
        <li class="mt-0.5 w-full">
          <a class="py-2.7 bg-blue-500/13 text-sm ease-nav-brand my-0 mx-2 flex items-center whitespace-nowrap rounded-lg px-4 font-semibold text-slate-700 transition-colors" href="../mapel/mapel.php">
            <div class="mr-2 flex h-8 w-8 items-center justify-center rounded-lg bg-center stroke-0 text-center xl:p-2.5">
              <i class="relative top-0 text-sm leading-normal text-blue-500 ni ni-tv-2"></i>
            </div>
  ...
  <main>
    <h1>Daftar Siswa</h1>
    <a href="tambah_siswa.php" style="display:inline-block; background:#28a745; color:white; padding:8px 12px; border-radius:4px; text-decoration:none; margin-bottom:10px;">+ Tambah Siswa</a>
        ...
        <?php if ($result && $result->num_rows > 0): ?>
          ...
          while ($row = $result->fetch_assoc()): ?>
            <tr>
              ...
              <td>
                <a href="edit_siswa.php?id=<?= $row['id_user'] ?>" style="margin-right: 6px; padding: 6px 10px; background: #ffc107; color: white; border-radius: 4px; text-decoration: none;">Edit</a>
                <a href="hapus_siswa.php?id=<?= $row['id_user'] ?>" onclick="return confirm('Yakin ingin menghapus siswa ini?');" style="padding: 6px 10px; background: #dc3545; color: white; border-radius: 4px; text-decoration: none;">Hapus</a>
              </td>
            </tr>
          <?php endwhile; ?>
        <?php else: ?>
          <tr>
            <td colspan="4" style="text-align:center;">Data siswa tidak ditemukan.</td>
          </tr>
        <?php endif; ?>
      </tbody>
    </table>
